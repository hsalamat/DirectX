//***************************************************************************************
// InstancingTexSkullApp.cpp using Anisotropic Sampler
//we use the SV_InstanceID semantic, which is a system value(notice the "SV_") that the GPU will provide for us, as input to a shader.
//Here is an example of a vertex shader that uses the SV_InstanceID semantic as input
// Moves the position of each instance along the positive x axiswe use the SV_InstanceID semantic, which is a system value (notice the "SV_") that the GPU will provide for us, as input to a shader. 
//Here is an example of a vertex shader that uses the SV_InstanceID semantic as input
// Then we move the position of each instance along the positive x axis
//***************************************************************************************
...
public:
	...
	TexSkullApp(const TexSkullApp& rhs) = delete;
	TexSkullApp& operator=(const TexSkullApp& rhs) = delete;
	~TexSkullApp();
...
void TexSkullApp::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void TexSkullApp::Draw(const GameTimer& gt)
{
...
void TexSkullApp::OnKeyboardInput(const GameTimer& gt)
{
}

void TexSkullApp::UpdateCamera(const GameTimer& gt)
{
...
void TexSkullApp::AnimateMaterials(const GameTimer& gt)
{

}

void TexSkullApp::UpdateObjectCBs(const GameTimer& gt)
{
...
void TexSkullApp::UpdateMaterialCBs(const GameTimer& gt)
{
...
void TexSkullApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void TexSkullApp::BuildRootSignature()
{
...
void TexSkullApp::BuildDescriptorHeaps()
{
	//
	// Create the SRV heap.
	//
...
void TexSkullApp::BuildShadersAndInputLayout()
{
	mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders\\VS_Instancing.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"Shaders\\PS_Instancing.hlsl", nullptr, "PS", "ps_5_1");
...
void TexSkullApp::BuildSkullGeometry()
{
...
void TexSkullApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	for (size_t i = 0; i < ritems.size(); ++i)
	{
		...
		//step1: draw three instances of skull - yup that easy!
		cmdList->DrawIndexedInstanced(ri->IndexCount, 3, ri->StartIndexLocation, ri->BaseVertexLocation, 0);
	}
}

std::array<const CD3DX12_STATIC_SAMPLER_DESC, 7> TexSkullApp::GetStaticSamplers()
{
