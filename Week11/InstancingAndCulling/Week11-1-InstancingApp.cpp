...
//step5: instead of UpdateObjectCBs, we do UpdateInstanceData
...
//step8: load all texture and apply to different skulls
...
void InstancingApp::BuildRootSignature()
{
	...
	//step8: Our root signature should represent how we bind our resources to registers

	// An array of textures, which is only supported in shader model 5.1+.  Unlike Texture2DArray, the textures
    // in this array can be different sizes and formats, making it more flexible than texture arrays.
	//Texture2D gDiffuseMap[7] : register(t0); --> slotRootParameter[3].InitAsDescriptorTable(1, &texTable

	// Put in space1, so the texture array does not overlap with these resources.  
	// The texture array will occupy registers t0, t1, ..., t6 in space0. 
	//StructuredBuffer<InstanceData> gInstanceData : register(t0, space1); --> slotRootParameter[0].InitAsShaderResourceView(0, 1)
	//StructuredBuffer<MaterialData> gMaterialData : register(t1, space1); --> slotRootParameter[1].InitAsShaderResourceView(1, 1)



	// Constant data that varies per pass.
	//cbuffer cbPass : register(b0)  --> slotRootParameter[2]
...
{
	//
	// step9: Create the SRV heap for 7 desciptors.
	//
...
//step11: build material for all 7 textures
...
void InstancingApp::BuildRenderItems()
{
	...
	// step12: Generate 125 instance data.
	const int n = 5;
...
{
	// Step13: For each render item...Note that we have only one item!
