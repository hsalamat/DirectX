...
void BoxApp::Update(const GameTimer& gt)
{
	...
	float angle = (-XM_PI + mTheta2) / 6.0f;
	float c = std::cos(angle);
	float s = std::sin(angle);

	//just to show you world and world1 are the same!
	XMMATRIX world1 (
	...
	XMMATRIX worldViewProj = world1 * view * proj;
	...
	//step5 the following does the same thing
	//struct ConstantBuff 
	//{
	//	XMMATRIX transform;
	//};

	//const ConstantBuff cb =
	//{
	//	{
	//		XMMatrixTranspose(world1 * view * proj)
	//	} 
	//};
	//XMStoreFloat4x4(&objConstants.WorldViewProj, cb.transform);
	//mObjectCB->CopyData(0, objConstants);
