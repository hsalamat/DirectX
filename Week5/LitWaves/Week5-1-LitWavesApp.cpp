...
void LitWavesApp::BuildRootSignature()
{
    ...
    slotRootParameter[2].InitAsConstantBufferView(2); //we need to update our root signature to support an additional constant buffer for materials. 
...
void LitWavesApp::BuildShadersAndInputLayout()
{
    ...
    mInputLayout =
    {
		...
		//step3: When we add a new vertex format, we need to describe it with a new input layout description:
...
//step4: Normal are calculated using partial derivatives: n = (-df/dx, 1, -df/dz)
