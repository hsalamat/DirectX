...
//step2: DDS texture loading code (DDSTextureLoader.h/.cpp) already supports loading cube maps, and we can load the texture like any other. 
...
//step3: When we create an SRV to a cube map texture resource, we specify the dimension D3D12_SRV_DIMENSION_TEXTURECUBE and use the TextureCube property of the SRV description:
...
void CubeMapApp::BuildMaterials()
{
	...
	//step6: Material for left/right spheres - highly reflective and black!
...
void CubeMapApp::BuildRenderItems()
{
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		//step7: apply mirror material to spheres
