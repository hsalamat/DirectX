...
private:
	...
	XMFLOAT3 mBoxTranslation = { 0.0f, 0.5f, -5.0f };
...
void StencilApp::Draw(const GameTimer& gt)
{
	...
	//the stencil buffer should be reset to some value at the beginning of each frame.
	//This is done with the following method(which also clears the depth buffer) :
...
void StencilApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	// step5: Update shadow world matrix.
	XMVECTOR shadowPlane = XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f); // xz plane
	XMVECTOR toMainLight = -XMLoadFloat3(&mMainPassCB.Lights[0].Direction);
	XMMATRIX S = XMMatrixShadow(shadowPlane, toMainLight);
	XMMATRIX shadowOffsetY = XMMatrixTranslation(0.0f, 0.001f, 0.0f);
...
void StencilApp::BuildPSOs()
{
	...
	//Comment the following line to modify the “Mirror” demo by not reversing the triangle winding order convention
...
void StencilApp::BuildRenderItems()
{
	...
	*reflectedSkullRitem = *skullRitem;
	...
	// step10: Shadowed skull will have different world matrix, so it needs to be its own render item.
	...
	*shadowedSkullRitem = *skullRitem;
	...
	XMStoreFloat4x4(&boxRitem->World, XMMatrixScaling(1.5f, 1.5f, 1.5f) * XMMatrixTranslation(0.0f, 0.5f, -5.0f));
	...
	*reflectedBoxRitem = *boxRitem;
	reflectedBoxRitem->ObjCBIndex = 8;
