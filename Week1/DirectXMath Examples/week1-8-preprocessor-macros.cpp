 ...
 *
 *
 * The Microsoft C/C++ compiler (MSVC) predefines certain preprocessor macros, 
 * depending on the language (C or C++), the compilation target, and the chosen compiler options.
...
#if (defined(_M_IX86)) //! @param _M_IX86 Defined as the integer literal value 600 for compilations that target x86 processors.
	cout << _M_IX86 << endl;;
#endif

#if (defined(_M_X64)) //! @param _M_X64 Defined as the integer literal value 100 for compilations that target x64 processors.
	cout << _M_X64 << endl;;
#endif

#if (defined(_M_ARM_))  //! @param _M_ARM Defined as the integer literal value 7 for compilations that target ARM processors. Otherwise, undefined.
	cout << _M_ARM_ << endl;;
#endif
...
//!Use __declspec(align(#)) to precisely control the alignment of user-defined data 
	...
	//! __declspec(align(32)) means that all static and automatic instances start on a 32-byte boundary.
