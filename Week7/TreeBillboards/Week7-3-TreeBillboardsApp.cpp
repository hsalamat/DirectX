...
void TreeBillboardsApp::BuildRenderItems()
{
	...
	canadaBoxRitem->Geo = mGeometries["shapeGeo"].get();
	canadaBoxRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	canadaBoxRitem->IndexCount = canadaBoxRitem->Geo->DrawArgs["sphere"].IndexCount;
	canadaBoxRitem->StartIndexLocation = canadaBoxRitem->Geo->DrawArgs["sphere"].StartIndexLocation;
	canadaBoxRitem->BaseVertexLocation = canadaBoxRitem->Geo->DrawArgs["sphere"].BaseVertexLocation;

	mRitemLayer[(int)RenderLayer::Opaque].push_back(canadaBoxRitem.get());
	...
	//step9 
	mAllRitems.push_back(std::move(canadaBoxRitem));
}
