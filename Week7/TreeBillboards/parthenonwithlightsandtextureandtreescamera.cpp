/** @file parthenonwithlight.cpp
 *  @brief adding lights to our parthenon.
 *
...
void ShapesApp::OnResize()
{
	...
	mCamera.SetLens(mFov, AspectRatio(), 1.0f, 1000.0f);
}
...
void ShapesApp::OnMouseMove(WPARAM btnState, int x, int y)
{
	...
	//	// Update the camera radius based on input.
		mFov += dx - dy;
		mFov = MathHelper::Clamp(mFov, 5.0f, 150.0f);
		mCamera.SetLens(mFov, AspectRatio(), 1.0f, 1000.0f);
	//	// Restrict the radius.
...
void ShapesApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.Lights[4].SpotPower = 2.0f;
...
void ShapesApp::BuildShapeGeometry()
{
	...
	auto totalVertexCount = box.Vertices.size() + box2.Vertices.size() + cylinder.Vertices.size();
...
void ShapesApp::BuildRenderItems()
{
	...
	XMStoreFloat4x4(&boxRitem->World, XMMatrixScaling(10.0f, 3.0f, 20.0f) * XMMatrixTranslation(0.0f, 0.5f, 5.0f));
	...
	XMStoreFloat4x4(&boxRitem2->World, XMMatrixScaling(9.5f, 1.5f, 19.0f) * XMMatrixTranslation(0.0f, 2.0f, 5.0f));
	...
	boxRitem2->Mat = mMaterials["tile0"].get();
	...
	XMStoreFloat4x4(&boxRitem3->World, XMMatrixScaling(8.5f, 1.0f, 18.0f) * XMMatrixTranslation(0.0f, 2.5f, 5.0f));
	//step22
	...
	boxRitem3->Mat = mMaterials["stone0"].get();
	...
	//XMStoreFloat4x4(&cylinderRitem->World, XMMatrixScaling(0.5f, 1.0, 0.5f) * XMMatrixTranslation(0.0f, 4.5f, 0.0f));
	...
	//XMStoreFloat4x4(&cylinderRitem2->World, XMMatrixScaling(0.5f, 1.0, 0.5f) * XMMatrixTranslation(2.0f, 4.5f, 0.0f));
	//cylinderRitem2->ObjCBIndex = 4;
	...
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 17; j++)
		{
			...
			XMStoreFloat4x4(&rItem[i][j]->World, XMMatrixScaling(0.5f, 1.0, 0.5f) * XMMatrixTranslation(-3.5 + i, 4.5f, -3.0f + j));
			//step24
			...
			rItem[i][j]->Mat = mMaterials["bricks0"].get();
	...
	XMStoreFloat4x4(&boxRitem4->World, XMMatrixScaling(8.5f, 1.0f, 18.0f) * XMMatrixTranslation(0.0f, 6.5f, 5.0f));
	XMStoreFloat4x4(&boxRitem4->TexTransform, XMMatrixScaling(1.0f, 1.0f, 1.0f));

	//boxRitem4->ObjCBIndex = 8 * 17 + 3;
	...
	XMStoreFloat4x4(&skullRitem->World, XMMatrixScaling(0.5f, 0.5f, 0.5f) * XMMatrixTranslation(0.0f, 1.0f, -4.0f));
	XMStoreFloat4x4(&skullRitem->TexTransform, XMMatrixScaling(1.0f, 1.0f, 1.0f));

	//skullRitem->TexTransform = MathHelper::Identity4x4();
	//skullRitem->ObjCBIndex = 8 * 17 + 4;
